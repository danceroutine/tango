...
export async function seedPosts(count = 1000): Promise<void> {
    ...
    if (existing >= count) {
        console.log(`[nuxt-blog bootstrap] Skipped; already have ${existing} posts.`);
        return;
    }
    ...
    console.log(`[nuxt-blog bootstrap] Seeded ${rows.length} posts (target total: ${count}).`);
}
...
main().catch((error: unknown) => {
    console.error('[nuxt-blog bootstrap] Failed:', error);
    process.exit(1);
});
