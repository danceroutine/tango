...
export class OffsetPaginator<T extends Record<string, unknown>> implements Paginator<T, T, OffsetPaginatedResponse<T>> {
    ...
    toResponse<TResult>(
        ...
        const response: OffsetPaginatedResponse<TResult> = { results: resolveQueryResultRows(results) };
    ...
    async getPage(page: number): Promise<Page<T>> {
        ...
        return new OffsetPage(resolveQueryResultRows(results), page, this.perPage, totalCount);
    }
